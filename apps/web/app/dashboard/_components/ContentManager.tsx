...
export default function ContentManager({ impersonating }: { impersonating: boolean }) {
  ...
  async function publish(status: "draft" | "published") {
    ...
    try {
      const r = await fetch("/api/creator/content", {
        ...
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title, contentType, body, pricePerBlock: price, summary, tags, status }),
      });
      const d = await r.json();
      if (r.ok) {
        ...
        toast(
          "success",
          status === "published" ? "Published — it's live on the marketplace now." : "Draft saved."
        );
      } else {
  ...
  async function toggle(id: string, status: string) {
    ...
    loadList();
    toast("success", next === "published" ? "Now live on the marketplace." : "Unpublished — hidden from the marketplace.");
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="card">
        ...
        <div className="flex flex-col gap-3">
          ...
          <input value={summary} onChange={(e) => setSummary(e.target.value)} placeholder="Summary (shown on the marketplace card + as the free intro)" className="rounded-lg border border-outline px-3 py-2 text-body-sm" />
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <button onClick={() => publish("published")} disabled={busy || disabled || !title || !body} className="btn-primary px-6 py-2">Publish to Marketplace</button>
        </div>
